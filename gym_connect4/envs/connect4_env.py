...
class Connect4Env(gym.Env):
    ...
    def step(self, actions: List[int]):
        ...
        # winner = self.get_winner() if game_over else WINNER_NONE
        obs = {
            'action_mask': self.get_action_mask(player),
            'board': self.get_state(player),
            # 'current_player': np.array([next_player]),
            # 'player_id': np.array([i]),
            # 'winner': winner,
        }
        rewards = self.game.get_reward(player)
